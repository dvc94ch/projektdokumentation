...
\section{Übersichtsmodell}
\label{sec:Modelluebersicht}

\section{Beschreibung der Komponenten}
\label{sec:KompBeschrieb}
%TODO CAD mit Nummern den Kapiteln entsprechend
...
\begin{figure}[h!]
	\centering
	\includegraphics[keepaspectratio,width=0.8\textwidth]{Ablaufdiagramm_ModusOperandi}
...
%TODO Bilder pro Ablaufschritt - Original, SW, Konturen, Rechtecke... you know what i mean

Die Erkennung läuft wie folgt ab: 

-	Video Stream eröffnen
-	Erhaltenes Bild wird durch einen binary threshold zu purem schwarz-weiss 
-	Konturen werden gesucht und erkannt
-	Rechtecke werden ausfindig gemacht und markiert
-	Sofern 6 Rechtecke den selben Mittelpunkt haben, ist es die Zielplatte
...
\section{Beschreibung Beziehung zwischen Komponenten}
\label{sec:KompBeziehung}

\subsubsection{Kommunikation zwischen Boards}
\label{sec:BoardKomm}
Für die Verwendung einer Kommunikation mittels einer asynchronen, seriellen UART-Schnittstelle können die jeweiligen PINs verwendet werden.

Auf dem RaspberryPi sind dies Pin 8 (Tx) und Pin 10 (Rx):
\begin{figure}[h!]
	\centering
...
\section{Software und Softwaresubsysteme}
\label{sec:SoftSoftSub}
...
% TODO MARKUS Motorenleistung berechnen/definieren
...
Um die Last zu greifen fiel unsere erste Wahl ursprünglich auf eine Konstruktion mit Elektromagnet. Diesen Elektromagneten wollten wir selbst bauen. Das Testprotokolle «Elektromagnet V1» zeigte, dass die Last zwar angehoben werden konnte, jedoch der Elektromagnet sehr heiss wurde auf Grund der Leistung von 4W. 

Nach weiteren Versuchen wurde im Testprotokoll «Elektromagnet V2» klar, dass durch die vorgenommenen Änderungen der Elektromagnet nur noch am Rand eine magnetische Wirkung besitzt.
...
Der Schrittmotor für den Antrieb ist alleine zur Bestimmung der Koordinaten verantwortlich. Mithilfe einer Tabelle, welche den Durchhang an den verschiedenen x-Positionen bestimmt, können die x- und y-Werte ausgelesen werden. Diese Werte sind für die Last Auf- und Abnahme und die Positionsanzeige per LCD-Display erforderlich.
